...
class CustomTimeline(QWidget):
    ...
    def paintEvent(self, event):
        ...
        # Draw annotations on the white background
        if self.total_frames:
            for annotation in self.annotations:
                ...
                start_x = rect.width() * (start_frame / self.total_frames)
                end_x = rect.width() * (end_frame / self.total_frames)

                painter.fillRect(QRect(start_x, 0, end_x - start_x, rect.height()), color)

        # Draw the time indicator
        if self.total_frames > 0:
            painter.setPen(QPen(QColor(216, 27, 96), 2))  # Magenta color for the time indicator
            current_x = rect.width() * (self.current_frame / self.total_frames)
        ...
        painter.drawRect(rect.adjusted(0, 0, -1, -1))  # Draw the border
...
class VideoAnnotationApp(QMainWindow):
    def __init__(self):
        ...
        # Add playback speed slider here
        ...
        # Connect the frameSelected signal to a method
        self.timeline.frameSelected.connect(self.set_frame)
        ...
        self.speed_slider.setMaximumHeight(20)  # You can adjust this value as needed
    ...
    def display_frame(self, frame):
        ...
        # Magic numbers for text and rectangle
        text_scale = 3
        text_thickness = 4
        rect_height = 80
        rect_margin = 10
        rect_width = 700  # Adjust as needed
        text_x = frame.shape[1] - rect_width - rect_margin
        text_y = 120

        # Check if there is a current annotation and it's a dictionary
        if self.current_annotation and isinstance(self.current_annotation, dict):
            annotation_type = self.current_annotation['type']
            color = (30, 136, 229) if annotation_type == "Upside Down" else (0, 77, 64)
    ...
    def open_video_file(self):
        ...
        if not os.path.isdir(default_dir):
            ...
            default_dir = os.path.dirname(os.path.abspath(__file__))  # Script's directory

        file_name, _ = QFileDialog.getOpenFileName(self, "Open Video File", default_dir, "Video Files (*.mp4 *.avi *.mov)")
        if file_name:
            self.load_video(file_name)
            self.current_video_file = file_name  # Store the path of the current video file
            self.load_annotations_button.setEnabled(True)  # Enable the load annotations button
            print(f'loading video file from: {file_name}')

    def annotate(self):
        new_annotation_type = self.sender().text()
        new_annotation_type = new_annotation_type.split(' (')[0]
        ...
        self.process_overlapping_annotations(current_frame, new_annotation_type)
    ...
    def process_overlapping_annotations(self, current_frame, new_annotation_type):
        for annotation in self.annotations:
            if annotation['start_frame'] <= current_frame <= annotation['end_frame']:
                # Adjust existing annotations based on the overlap
                if current_frame - annotation['start_frame'] > 0:
                    annotation['end_frame'] = current_frame - 1
                else:
                    self.annotations.remove(annotation)

                # If the overlapping annotation extends beyond the current frame, split it
                ...
                # Remove annotations that become invalid after adjustment
                self.annotations = [a for a in self.annotations if a['start_frame'] < a['end_frame']]
